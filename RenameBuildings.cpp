...
{
    MyGUI::EditBox *edit = dynamic_cast<MyGUI::EditBox *>(g_rename_window->findWidget("RenameBuildingEdit"));
    if (!edit)
    {
    ...
    if (start == std::string::npos) { new_name.clear(); }
    else
    {
...
void OnCloseRenameWindow(MyGUI::Window *sender, const std::string &name) { g_rename_window->setVisible(false); }
...
void OnShowRenameWindow(MyGUI::WidgetPtr sender)
{
    ...
    Building *building = ou->player->selectedObject.getBuilding();
    if (building && building->isThePlayer() && building->getBuildingClass() != BCTYPE_DOOR)
    {
        DebugLog("Showing rename window");
        MyGUI::EditBox *edit = dynamic_cast<MyGUI::EditBox *>(g_rename_window->findWidget("RenameBuildingEdit"));
        if (edit) { edit->setCaption(building->getName()); }
        g_rename_window->setVisible(true);
    }
}

// Drag start: record button position and mouse offset at press
void OnRenameButtonMousePress(MyGUI::WidgetPtr sender, int left, int top, MyGUI::MouseButton id)
{
...
void OnRenameButtonMouseDrag(MyGUI::WidgetPtr sender, int left, int top, MyGUI::MouseButton id)
{
    if (g_dragging && id == MyGUI::MouseButton::Left)
    {
        ...
        if (new_left + button_size.width > parent_size.width) { new_left = parent_size.width - button_size.width; }
        if (new_top + button_size.height > parent_size.height) { new_top = parent_size.height - button_size.height; }
...
// Drag end
void OnRenameButtonMouseRelease(MyGUI::WidgetPtr sender, int left, int top, MyGUI::MouseButton id)
{
    if (id == MyGUI::MouseButton::Left) { g_dragging = false; }
}
...
TitleScreen *TitleScreen_hook(TitleScreen *thisptr)
{
    ...
    g_rename_window = gui->createWidgetReal<MyGUI::Window>(
        "Kenshi_WindowCX", 0.25f, 0.35f, 0.30f, 0.072f, MyGUI::Align::Center, "Window", "RenameBuildingWindow"
    );
    g_rename_window->setCaption("Rename Building");
    g_rename_window->setVisible(false);
    g_rename_window->eventWindowButtonPressed += MyGUI::newDelegate(OnCloseRenameWindow);

    MyGUI::EditBox *edit = g_rename_window->getClientWidget()->createWidgetReal<MyGUI::EditBox>(
        "Kenshi_EditBox", 0.05f, 0.1f, 0.68f, 0.7f, MyGUI::Align::Default, "RenameBuildingEdit"
    );
    edit->setCaption("");

    MyGUI::Button *confirm_rename_button = g_rename_window->getClientWidget()->createWidgetReal<MyGUI::Button>(
        "Kenshi_Button1", 0.75f, 0.1f, 0.19f, 0.7f, MyGUI::Align::Center, "ConfirmRenameBuildingButton"
    );
    ...
    g_show_rename_window_button = gui->createWidgetReal<MyGUI::Button>(
        "Kenshi_Button1", 0.01f, 0.75f, 0.06f, 0.02f, MyGUI::Align::Default, "Window", "ShowRenameBuildingWindowButton"
    );
    g_show_rename_window_button->setCaption("Rename");
    g_show_rename_window_button->setVisible(false);
    g_show_rename_window_button->eventMouseButtonClick += MyGUI::newDelegate(OnShowRenameWindow);
    g_show_rename_window_button->eventMouseButtonPressed += MyGUI::newDelegate(OnRenameButtonMousePress);
    g_show_rename_window_button->eventMouseDrag += MyGUI::newDelegate(OnRenameButtonMouseDrag);
    g_show_rename_window_button->eventMouseButtonReleased += MyGUI::newDelegate(OnRenameButtonMouseRelease);
